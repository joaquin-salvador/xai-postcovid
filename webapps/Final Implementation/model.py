...
def render_overview(model, X_test, y_test, class_names, precomputed_preds):
    ...
    st.markdown(
        ...
        "**3. LightGBM Surrogate**\n"
        "- A LightGBM classifier (500 trees, depth 6, lr=0.05) is trained to mimic TabPFN's "
        "predictions on the training set\n"
        "- Surrogate fidelity: **96.8% agreement** on the test set, Spearman rank correlation "
        "**0.979** with TabPFN's probability outputs\n"
        "- The surrogate enables **exact TreeSHAP** computation (polynomial time), which would "
        "not be feasible directly on TabPFN\n"
        "- The surrogate also powers the **What-If Analysis** for instant interactive predictions"
    )
